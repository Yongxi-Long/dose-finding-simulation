...
gen_profile <- function(drug_profile,dose_levels)
{
  ...
  source(paste0("profiles/",attr(drug_profile,"label"),".R"))
  sapply(dose_levels,function(dose) drug_profile(dose))
}
                     ...
                     )
{
  ...
  data_list <- sapply(covs_df, function(x)
    ...
    sapply(final_args_alldoses, function(final_args_thisdose)
    {
      ...
      data <- do.call(simsurv,final_args_thisdose)
